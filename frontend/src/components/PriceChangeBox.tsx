...
export default function PriceChangeBox({
  ...
  return (
    ...
    >
      <p
        className={`${priceChange! >= 0 ? "text-[#21c45d] dark:text-emerald-400" : "text-[#ef4444] dark:text-rose-400"} text-${fontSize} font-bold`}
      >
        {priceChange! > 0 ? "↑" : priceChange! < 0 ? "↓ " : ""}{" "}
        {numToMoney(priceChange!, true)}%
      </p>
      <p
        className={`${priceChange! >= 0 ? "text-[#21c45d] dark:text-emerald-400" : "text-[#ef4444] dark:text-rose-400"} text-${fontSize}`}
      >
        24h
      </p>
    </div>
