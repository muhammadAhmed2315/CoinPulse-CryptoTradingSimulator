import { getAllCoinNamesDict, formatFloatToUSD, formatUNIXTimestamp, } from "../js/helpers.js";
 ...
 */
function resetChartButtons() {
    document.querySelector(".chart-btn--balance").style.backgroundColor = "#17C671";
    document.querySelector(".chart-btn--assets").style.backgroundColor = "#17C671";
...
function displayTransactionData() {
    ...
    if (tableRows.length === 0 &&
        ...
        for (const [i, row] of Array.from(tableRows).entries()) {
            ...
            row.querySelector(".transaction-quantity").setAttribute("title", transactionsData[i].quantity.toLocaleString("en-US", {
                maximumFractionDigits: 100,
            }));
            ...
            row
                ...
                .setAttribute("title", "$" + formatFloatToUSD(transactionsData[i].price_per_unit, 2));
            // Update transaction price at execution
            const priceAtExecution = transactionsData[i].price_at_execution != -1
            ...
            row.querySelector(".transaction-time").textContent = formatUNIXTimestamp(transactionsData[i].timestamp);
            row
                .querySelector(".transaction-time")
                .setAttribute("title", formatUNIXTimestamp(transactionsData[i].timestamp));
            // Update action
            if (transactionsData[i].status === "open") {
                // Add cancel button
                ...
                row
                    ...
                    const result = await requestOpenTradeCancellation(Number(transactionsData[i].id));
                    if (result) {
                        // Change cross button to cancelled text
                        row.querySelector(".transaction-action").innerHTML =
                            "<p>Cancelled</p>";
                        // Change colour of status bubble
                        row.querySelector(".transaction-status-bubble").style.backgroundColor = "#EB5757";
                    }
                });
            }
            else {
...
function addSortingEventListeners() {
    ...
    for (const heading of headingInfo) {
        const headerP = document.querySelector(`.my-trades-table__header-${heading.cssClass} p`);
        if (!headerP)
            continue;
...
main();
//# sourceMappingURL=my-trades.js.map