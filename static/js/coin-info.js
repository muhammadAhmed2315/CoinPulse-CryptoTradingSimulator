...
import { getAllCoinNamesDict, formatFloatToUSD, scrollToSection, } from "../js/helpers.js";
// ========================
// ===== INITIAL DATA =====
// ========================
...
function addSearchBarEventListeners() {
    ...
    document
        ...
        const resultsContainer = document.querySelector(".coin-info__search-results");
        if (!input) {
    ...
    document
        ...
        if (targetElement.classList.contains("result-item")) {
            if (targetElement.textContent !== "No results found") {
                ...
                document.querySelector(".coin-info__search-input").value = selectedCoinName;
    ...
    document.addEventListener("click", function (e) {
        if (!e.target.classList.contains("coin-info__search") &&
...
async function addSecondNavButtonEventListeners() {
    ...
    document.querySelector(".sn-btn--ohlc").style.borderBottom = "1.6px solid #17c671";
 ...
 */
function updateCoinInfo() {
    const coinImg = document.querySelector(".coin-name img");
    const coinPriceChangeImg = document.querySelector(".coin-price-change img");
    const coinPriceChangePara = document.querySelector(".coin-price-change p");
    const coinMarketCapImg = document.querySelector(".coin-market-cap img");
    ...
    if (currentCoin.price_change_percentage_24h >= 0) {
        coinPriceChangeImg.setAttribute("src", "../../static/img/icons/arrow-up.svg");
        ...
        coinPriceChangeImg.setAttribute("src", "../../static/img/icons/arrow-down.svg");
        coinPriceChangeImg.style.filter =
    ...
    if (currentCoin.market_cap_change_percentage_24h >= 0) {
        ...
        coinMarketCapImg.style.filter =
            "brightness(0) saturate(100%) invert(64%) sepia(13%) saturate(6613%) hue-rotate(105deg) brightness(97%) contrast(82%)";
        document.querySelector(".coin-market-cap .percentage-change").style.color = "#17C671";
    }
    else {
        coinMarketCapImg.setAttribute("src", "../../static/img/icons/arrow-down.svg");
        coinMarketCapImg.style.filter =
            "brightness(0) saturate(100%) invert(38%) sepia(8%) saturate(7345%) hue-rotate(322deg) brightness(111%) contrast(82%)";
        document.querySelector(".coin-market-cap .percentage-change").style.color = "#EB5757";
    }
    ...
    document.querySelector(".coin-ath img").style.filter =
        "brightness(0) saturate(100%) invert(38%) sepia(8%) saturate(7345%) hue-rotate(322deg) brightness(111%) contrast(82%)";
    document.querySelector(".coin-ath .percentage-change").style.color = "#EB5757";
    ...
    document.querySelector(".coin-atl .percentage-change").style.color = "#17C671";
...
function renderNewsArticles() {
    ...
    for (const article of newsArticlesToRender) {
      ...
      <p class="news-post__author">Cryptopolitan</p>
        ...
        newsPost.querySelector(".news-post__title").href =
            article.url;
...
async function getAndRenderNewsArticles() {
    newsArticlesToRender = await getNewsArticles(currentCoin.name, newsArticlesCurrentPage);
    renderNewsArticles();
...
function renderRedditPosts() {
    ...
    if (seeMoreRedditButton)
        seeMoreRedditButton.remove();
    const markup = `
...
async function getAndRenderRedditPosts() {
    const afterPost = redditPostsToRender.length > 0 ? redditPostsToRender.at(-1).fullname : "";
