...
interface PrivateFeedPost extends GlobalFeedPost {
  visibility: boolean;
}

type LikeResponse = [{ currLikes: number; success: string }, number];

type TrendingCoinInfo = {
  id: string;
  name: string;
  symbol: string;
  large: string;
...
type WalletAssets = Record<
  string,
  { img: string; name: string; quantity: number }
>;
 ...
 */
async function fetchFeedPosts(type: string, page: number): Promise<void> {
  const fetchOptions = {
    method: "POST",
    headers: {
 ...
 * @returns {void}
 */
function renderFeedPosts(type: string): void {
  const markup = `
  ...
  let currFeedPosts: GlobalFeedPost[] | PrivateFeedPost[] = [];
  ...
  for (let i = 0; i < currFeedPosts.length; i++) {
    ...
    let icon: HTMLElement | null = div.querySelector(".like-btn");
    if (icon) {
    ...
    if (currFeedPost.transaction_type === "buy") {
      ...
      (div.querySelector(".order-quantity") as HTMLElement)!.style.color =
        "#17c671";
    } else {
      div.querySelector(".order-quantity")!.textContent =
        "-" + currFeedPost.quantity.toFixed(4);
    ...
    if (currFeedPost.comment) {
      ...
      (div.querySelector(".feedpost-comment p") as HTMLElement)!.textContent =
    ...
    div
      ...
      .addEventListener("click", async function (this: HTMLElement) {
...
async function getTrendingCoinsData() {
  ...
  try {
    ...
    const res = data.map((coin: any) => coin.item);
 ...
 */
async function renderTrendingCoins(coinsData: TrendingCoinInfo[]) {
  const markup = `
  ...
  for (const coin of coinsData) {
    ...
    (newCard.querySelector(
      ".trending-coins-card__header .img--first",
    ) as HTMLImageElement)!.src = coin.large;
    (newCard.querySelector(
      ".trending-coins-card__main .img--third",
    ) as HTMLImageElement)!.src = coin.large;
    ...
    if (coin.data.price_change_percentage_24h.usd >= 0) {
      (newCard.querySelector(
        ".trending-coins-card__main .coin-price-change",
      ) as HTMLParagraphElement)!.style.color = "#17c671";
    } else {
      (newCard.querySelector(
        ".trending-coins-card__main .coin-price-change",
      ) as HTMLParagraphElement)!.style.color = "#EB5757";
    }
 ...
 */
function formatWalletAssetsData(
  coin_info: { id: string; name: string; image: string }[],
  coin_quantities: Record<string, number>,
) {
  const res: WalletAssets = {};
 ...
 */
function renderWalletAssets(assets: WalletAssets, balance: number): void {
  ...
  for (const coin in assets) {
    ...
    // Update coin image
    (newTableRow.querySelector(".holdings-data img") as HTMLImageElement).src =
      img;
 ...
 */
async function getOpenTradesData(): Promise<OpenTrade[] | undefined> {
  const fetchOptions = {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
  ...
  const data: { data: OpenTrade[]; success: string } = await response.json();
 ...
 */
function formatOpenTradesData(data: OpenTrade[]): OpenTradesInfo {
  const res: OpenTradesInfo = {
    limitBuy: [],
    limitSell: [],
    stopBuy: [],
    stopSell: [],
 ...
 */
function getUniqueCoins(trades: OpenTradesInfo): string[] {
  const uniqueCoins = new Set<string>();

  for (const tradeType in trades) {
    for (const trade of trades[tradeType as keyof OpenTradesInfo]) {
 ...
 */
function getOpenTradesMarkup(): string {
  return `
  <div class="first">
...
function renderOpenTrades(
  ...
  for (const orderType in trades) {
    if (trades[orderType as keyof OpenTradesInfo].length == 0) {
    ...
    for (const trade of trades[orderType as keyof OpenTradesInfo]) {
      ...
      (
        tradeInfoDiv.querySelector(
          ".order-price-info img:first-of-type",
        ) as HTMLImageElement
      ).src = coinInfo.img;
...
async function requestOpenTradeCancellation(
  ...
  else if (data.error) return false;
  return;
}
 ...
 *                          and rendered.
 */
async function getAndRenderOpenTradesData(): Promise<void> {
  const openTradesData = await getOpenTradesData();
  if (!openTradesData) return;
 ...
 *   "own").
 */
function addFeedMenuButtonEventListeners(): void {
  const globalFeedBtn: HTMLParagraphElement = document.querySelector(
    ".feed-header__global-label",
  )!;
  const ownFeedBtn: HTMLParagraphElement = document.querySelector(
    ".feed-header__own-label",
  )!;
  const globalFeedContainer: HTMLDivElement = document.querySelector(
    ".global-feedposts-container",
  )!;
  const ownFeedContainer: HTMLDivElement = document.querySelector(
    ".own-feedposts-container",
  )!;
 ...
 */
function addTrendingCoinsDraggableEventListener(): void {
  ...
  function beginMomentumScrolling(): void {
  ...
  function cancelMomentumScrolling(): void {
 ...
 */
function addOverviewOpenButtonsEventListeners(): void {
  const overviewBtn: HTMLParagraphElement = document.querySelector(
    ".positions-header__overview-label",
  )!;
  const openPositionsBtn: HTMLParagraphElement = document.querySelector(
    ".positions-header__open-positions-label",
  )!;

  const overviewCard: HTMLDivElement = document.querySelector(
    ".portfolio-overview-card",
  )!;
  const openPositionsCard: HTMLDivElement = document.querySelector(
    ".open-positions-card",
  )!;
 ...
 */
async function getCoinDataFromAPI(coin_ids: string[]): Promise<CoinInfo[]> {
 ...
 */
function showNewTradeSidebarOnLoad(): void {
