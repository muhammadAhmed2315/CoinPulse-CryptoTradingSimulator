...
let assetsValueHistory: [number, number][] = [];
let balanceHistory: [number, number][] = [];
let totalValueHistory: [number, number][] = [];
 ...
 */
function addChartButtonEventListeners(): void {
 ...
 */
function resetChartButtons(): void {
  (
    document.querySelector(".chart-btn--balance") as HTMLParagraphElement
  ).style.backgroundColor = "#17C671";
  (
    document.querySelector(".chart-btn--assets") as HTMLParagraphElement
  ).style.backgroundColor = "#17C671";
  (
    document.querySelector(".chart-btn--total") as HTMLParagraphElement
  ).style.backgroundColor = "#17C671";
}
 ...
 */
function createTableRows(numRows = 25): void {
  const markup = `
  ...
  if (
    ...
    for (const [i, row] of Array.from(tableRows).entries()) {
      ...
      if (transactionsData[i]!.status === "open") {
        ...
        (
          row.querySelector(".transaction-action") as HTMLTableCellElement
        ).style.padding = "0";
 ...
 *   being sorted
 */
function addSortingEventListeners(): void {
  ...
  function resetAllTableHeadings() {
    (
      sortDOMElementDict[
        currentSort as keyof typeof sortDOMElementDict
      ][0] as HTMLParagraphElement
    ).textContent =
      sortDOMElementDict[currentSort as keyof typeof sortDOMElementDict][1] +
      "\u00A0\u00A0";
  }
  ...
  for (const heading of headingInfo) {
    ...
    headerP.addEventListener(
      "click",
      async function (this: HTMLTableCellElement) {
