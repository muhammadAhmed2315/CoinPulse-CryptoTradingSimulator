...
type CoinDataCache = {
  market_cap_asc: CoinInfo[] | null;
  market_cap_desc: CoinInfo[] | null;
  volume_asc: CoinInfo[] | null;
  volume_desc: CoinInfo[] | null;
};

let current_page = 1;
let current_sort: keyof CoinDataCache = "market_cap_desc";
let coinData: CoinDataCache = {
  market_cap_asc: null,
...
async function fetchCoinsData(
  ...
  try {
    ...
    const results = data.map((coin: RawCoinInfo) => {
      let {
        image,
        name,
        symbol,
 ...
 */
function createCoinTableRows(numRows: number = 10): void {
  const markup = `
    <td>
 ...
 */
function displayCoins(pageNum: number): void {
  ...
  for (const [i, row] of Array.from(tableRows).entries()) {
    ...
    const priceChange1hElem = row.querySelector<HTMLTableCellElement>(
    ...
    const priceChange24hElem = row.querySelector<HTMLTableCellElement>(
    ...
    const priceChange7dElem = row.querySelector<HTMLTableCellElement>(
      ".coin-price-change-7d",
    );
 ...
 */
function addPaginationButtonEventListeners(): void {
  const tableButtonNext: HTMLParagraphElement | null = document.querySelector(
    ".pagination-button--next",
  );
  const tableButtonPrevious: HTMLParagraphElement | null =
    document.querySelector(".pagination-button--previous");

  if (!tableButtonNext || !tableButtonPrevious) return;
 ...
 */
function addSortDropdownEventListeners(): void {
  const sortDropdown: HTMLSelectElement | null = document.querySelector(
    ".sort-dropdown-menu",
  );
 ...
 */
function resetPaginationButtons(): void {
  const prevBtn: HTMLParagraphElement | null = document.querySelector(
    ".pagination-button--previous",
  );
  const nextBtn: HTMLParagraphElement | null = document.querySelector(
    ".pagination-button--next",
  );
